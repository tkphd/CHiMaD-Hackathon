...
if path.isfile('../energy_dx02.tsv'):
    t, e = np.loadtxt('../energy_dx02.tsv',usecols=(0, 1), unpack=True, skiprows=2, delimiter='\t')
...
plt.figure(2)
fnames = sorted(glob.glob("../data/electrochem_dx10.*.csv"))
if len(fnames) > 0:
    n = 0
    for file in fnames:
...
plt.figure(3)
fnames = sorted(glob.glob("../data/electrochem_dx05.*.csv"))
if len(fnames) > 0:
    n = 0
    for file in fnames:
        num = int(re.search('[0-9]{3,6}', file).group(0)) / 20
...
plt.figure(4)
fnames = sorted(glob.glob("../data/electrochem_dx02.*.csv"))
if len(fnames) > 0:
    n = 0
    for file in fnames:
        num = int(re.search('[0-9]{3,6}', file).group(0)) / 20
    ...
    plt.legend(bbox_to_anchor=(1.05, 1), loc=2, borderaxespad=0.)
    plt.savefig('comp_dx02.png', bbox_inches='tight', dpi=400)
    plt.close()
